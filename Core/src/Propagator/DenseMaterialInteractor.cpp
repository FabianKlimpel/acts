...
void Acts::DenseMaterialInteractor::reset(double q,
                                          double momentum,
                                          double time,
                                          Vector3D direction,
...
Acts::BoundSymMatrix Acts::DenseMaterialInteractor::evaluateMultipleScattering(
    int pdg, float mass, float q, double momentum,
    double time, result_type& result) const {
  ...
  const double s2 = result.s * result.s;
  const double invSinTheta =
      result.dir0.norm() / VectorHelpers::perp(result.dir0);

  const double qop = (q == 0. ? 1. : q) / momentum;
  const double deltaQop = qop - result.qop0;
  const double deltaT = time - result.t0;
